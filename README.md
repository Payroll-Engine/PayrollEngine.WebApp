...
To run the web application server, the web host must support the execution of .NET Core applications. For local development, [IIS Express](https://learn.microsoft.com/en-us/iis/extensions/introduction-to-iis-express/iis-express-overview) serves as host in two execution variants:
...
| `AutoLogin` | Automatic login using the `StartupTenant` and `StartupUser` (bool) <sup>1)</sup> | `false` |
| `StartupTenant` | The start tenant using th auto login option (string) <sup>1)</sup> | - |
| `StartupUser` | The start user using the auto login option (string) <sup>1)</sup> | - |
...
| `AppImageDarkMode` | The application image for the dark mode (string) | Payroll Engine image |
...
All other characters: Literals. All non-mask elements (literals) will appear as themselves within masked text box.
